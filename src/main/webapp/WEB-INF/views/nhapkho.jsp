      ...
      <%@ include file="_head.jspf" %>
        ...
        <c:choose>
          <c:when test="${not empty current}">
            <a class="btn gray sm" href="${pageContext.request.contextPath}/admin/receipts">← Danh sách phiếu nhập</a>
            <h2>Phiếu nhập #${current.receiptId}
              <span class="badge ${current.pending ? 'pending' : 'approved'}">${current.pending ? 'Chờ duyệt' : 'Đã
                duyệt'}</span>
            </h2>
            <div class="panel">
              <p><b>Nhà cung cấp:</b> ${current.supplierName} &nbsp;|&nbsp;
              ...
              <div class="actions">
                <form class="inline-form" method="post" action="${pageContext.request.contextPath}/admin/receipts"
                  ...
                  <input type="hidden" name="action" value="approve"><input type="hidden" name="id"
                    value="${current.receiptId}">
                  <button class="btn green" type="submit">✔ Duyệt phiếu nhập</button>
                </form>
                <form class="inline-form" method="post" action="${pageContext.request.contextPath}/admin/receipts"
          ...
          <c:otherwise>
            <div class="panel">
              <form method="post" action="${pageContext.request.contextPath}/admin/receipts">
                ...
                <div class="form-grid">
                  <div><label>Nhà cung cấp</label><input type="text" name="supplierName" required></div>
                  <div><label>Ghi chú</label><input type="text" name="remarks"></div>
                </div>
                <div class="actions"><button class="btn" type="submit">+ Tạo phiếu nhập</button></div>
              </form>
            </div>
              ...
              <c:forEach var="r" items="${receipts}">
                <tr>
                  ...
                  <td><span class="badge ${r.pending ? 'pending' : 'approved'}">${r.pending ? 'Chờ duyệt' : 'Đã
                      duyệt'}</span></td>
                  <td class="r"><a class="btn sm gray" href="?view=${r.receiptId}">Chi tiết</a></td>
                </tr>
              </c:forEach>
            </table>
          </c:otherwise>
